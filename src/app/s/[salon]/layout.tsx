...
import { SalonNav, type NavGroup } from '@/components/salon/salon-nav'
...
export default async function SalonLayout({
   ...
   * nothing under it, which is how a stylist ended up with an "Admin" label
   * above blank space.
   */
  const groups: NavGroup[] = isStaff
    ? STAFF_NAV.map((group) => ({
        label: group.label,
        items: group.items.filter((item) => permitted(ctx, item.action)),
      })).filter((group) => group.items.length > 0)
    : [{ label: null, items: CLIENT_NAV }]
   ...
   * so overriding the variables re-skins buttons, links, focus rings, washes
   * and the navy of the nav column at once — no component knows this happened.
   * Gold and rose are deliberately untouched: their meaning (money, and the
   * hair itself) is the same at every salon and is not the salon's to change.
   */
  ...
  return (
    ...
    >
      <SalonNav
        ...
        kicker={isStaff ? 'Salon' : 'Your account'}
        groups={groups}
        footnote={`Times shown in ${ctx.timezone.replace('_', ' ')}`}
      />

      <div className="app-wash flex min-w-0 flex-1 flex-col">
        ...
        {/*
         * Mobile only. On a wide screen the nav column already carries the
         * salon's name at the top and its timezone at the bottom, and printing
         * both again here was the same sentence twice on one page.
         */}
        <footer className="border-t border-line bg-canvas/70 lg:hidden">
...
/**
 * The staff nav, grouped by what somebody is actually trying to do.
 *
 * Sixteen links in one flat list is a list nobody reads to the end. The
 * sections are the day in front of you, the people you look after, the salon as
 * a business, and the settings you touch once a quarter — roughly the order a
 * day goes in.
 */
const STAFF_NAV: { label: string; items: { href: string; label: string; action: Action }[] }[] = [
  {
    label: 'The day',
    items: [
